...
## Acceptance Criteria
- [ ] Flatpak manifest file defines complete build process
- [ ] `make flatpak` builds the Flatpak bundle
- [ ] `make flatpak-install` installs the bundle locally for testing
- [ ] Application appears in desktop application menu after install
- [ ] Launching from application menu starts the full application
- [ ] Application has appropriate permissions (network for localhost, GPU access)
- [ ] Desktop entry file includes proper metadata (name, icon, categories)
- [ ] Application icon displays correctly in application menu
- [ ] `flatpak run <app-id>` launches the application

...
## Acceptance Criteria
- [ ] `make` (default target) builds the complete Electron application with bundled Go binary
- [ ] `make run` builds and launches the Electron application
- [ ] `make weave` builds only the Go server binary
- [ ] `make electron` builds only the Electron package (assumes Go binary exists)
- [ ] `make clean` removes all build artifacts including Electron dist folder
- [ ] Go binary is copied into Electron's resources directory during build
- [ ] Build works from clean checkout (no pre-existing artifacts required)
- [ ] Build fails with clear error if npm dependencies are missing

...
## Acceptance Criteria
- [x] l5-relay reads JSON objects from stdin one at a time
- [x] Each JSON object is POSTed to `$ORCHESTRATOR_URL/api/agent/journal`
- [x] `RUN_ID` from environment variable is included with each request
- [x] l5-relay exits cleanly when stdin closes (exit code 0)
- [x] POST failures are logged to stderr and do not stop processing (best-effort delivery)
- [x] l5-relay builds as a static binary (`go build`)
- [x] Unit tests cover: valid JSON input, malformed JSON handling, stdin close, POST failure resilience

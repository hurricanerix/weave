...
## Acceptance Criteria
- [ ] weave creates the Unix socket before spawning compute
- [ ] weave passes the socket path to compute via CLI argument
- [ ] compute connects to the existing socket (does not create it)
- [ ] compute monitors both the socket connection and stdin for closure
- [ ] compute terminates cleanly if either the socket closes or stdin closes
- [ ] When weave exits (clean or crash), compute terminates within 2 seconds
- [ ] No orphaned compute processes remain after weave exits
- [ ] No stale socket files remain after normal shutdown
- [ ] Socket creation errors are reported clearly to the user
- [ ] Existing image generation functionality continues to work
...
## Notes
This inverts the current ownership model:
- **Before:** compute creates socket, weave connects
- **After:** weave creates socket, compute connects
...
### 003: Replace socket_create with socket_connect in compute
**Domain:** compute
**Status:** pending
**Depends on:** 001, 002

Modify `compute-daemon/src/main.c` to use `socket_connect()` instead of `socket_create()` when a socket path is provided via CLI. The daemon should connect to the existing socket created by weave. Remove the call to `socket_cleanup()` in the cleanup function since compute no longer owns the socket file. Update error messages to indicate connection failures rather than creation failures.
...
Create a new function in `internal/startup/init.go` that creates the Unix socket before spawning compute. The function should construct the socket path from XDG_RUNTIME_DIR, create the socket directory with mode 0700, create and bind the socket, and start listening. Store the listening socket file descriptor for later use. Handle and report socket creation errors clearly to the user. This uses Go's `net.Listen("unix", socketPath)`.
...
### 007: Add socket passing to compute connection in weave
**Domain:** weave
**Status:** pending
**Depends on:** 006

Modify `internal/client/socket.go` to accept connections on the pre-created listening socket instead of dialing a socket file. Add a new function `AcceptConnection(listener net.Listener)` that accepts a connection from the listener and returns a `*Conn`. Update the web handlers or protocol layer to use the listening socket for accepting compute connections. Remove or deprecate the `Connect()` function that dials the socket file.
...
Update `test/integration/socket_test.go` to test the new parent-child lifecycle pattern. Remove the manual daemon spawning code. Add tests that verify: weave creates socket before compute spawns, compute connects to existing socket, compute terminates when weave exits, no orphaned processes remain, no stale socket files remain after shutdown. Update helper functions like `startDaemon()` to work with the new model.

---

### 010: Update compute socket tests for connection mode
**Domain:** compute
**Status:** pending
**Depends on:** 001

Update `compute-daemon/test/test_socket.c` to test the new `socket_connect()` function. Add tests that verify: connection to existing socket succeeds, connection to non-existent socket fails with appropriate error, connection to socket created by another process succeeds. Ensure existing `socket_create()` tests still pass for backward compatibility.
